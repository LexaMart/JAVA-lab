...
public class GraphicsDisplay extends JPanel {
    ...
    private Double[][] secondGraphicData;
    private Double[][] toCountSize;
...
// Шрифт для подписей осей координат
    ...
    public void addGraphic(Double[][] second) {
        this.secondGraphicData = second;
        this.toCountSize = new Double[this.secondGraphicData.length + this.graphicsData.length][];
        System.arraycopy(this.graphicsData, 0, this.toCountSize, 0, this.graphicsData.length);
        System.arraycopy(this.secondGraphicData, 0, this.toCountSize, this.graphicsData.length, this.secondGraphicData.length);
        repaint();
    }
    
    public void showGraphics(Double[][] graphicsData) {
// Сохранить массив точек во внутреннем поле класса
        this.toCountSize = null;
        this.secondGraphicData = null;
...
// Шаг 9 - Восстановить старые настройки холста
    ...
    public void paintDuoComponent (Graphics g){
        super.paintComponent(g);
        if (toCountSize == null || toCountSize.length == 0) return;
        minX = toCountSize[0][0];
        maxX = toCountSize[toCountSize.length - 1][0];
        minY = toCountSize[0][1];
        maxY = minY;
        for (int i = 1; i < toCountSize.length; i++) {
            if (toCountSize[i][1] < minY) {
                minY = toCountSize[i][1];
            }
            if (toCountSize[i][1] > maxY) {
                maxY = toCountSize[i][1];
            }
        }
        if (!turnGraph) {
            double scaleX = getSize().getWidth() / (maxX - minX);
...
// Отобразить график
    ...
    protected void paintDuoGraphics(Graphics2D canvas) {
        canvas.setStroke(graphicsStroke);
        canvas.setColor(Color.magenta);
        GeneralPath graphics = new GeneralPath();
        for (int i = 0; i < graphicsData.length; i++) {
...
//        Point2D.Double movePoint = xyToPoint(secondGraphicData[0][0], secondGraphicData[0][1]);
//        graphics.lineTo(movePoint.getX(), movePoint.getY());
        GeneralPath graphics2 = new GeneralPath();
        for (int i = 0; i < secondGraphicData.length; i++) {
            Point2D.Double secpoint = xyToPoint(secondGraphicData[i][0], secondGraphicData[i][1]);
            if (i > 0) {
                graphics2.lineTo(secpoint.getX(), secpoint.getY());
            } else {
                graphics2.moveTo(secpoint.getX(), secpoint.getY());
            }
        }
        canvas.draw(graphics);
    ...
    protected void paintIntegrals(Graphics2D canvas) {
        ...
        Double domens = 0.0;
        Double[][] array = this.toCountSize;
        if (this.toCountSize != null) array = this.toCountSize;
        GeneralPath path = new GeneralPath();
        for (int i = 0; i < array.length - 1; i++) {
            System.out.println("X: " + array[i][0] + " Y: " + array[i][1] + " i: " + i);
            if ((array[i][1] < 0 == array[i + 1][1] >= 0) || (array[i][1] == 0)) {
                if (domens != 0) {
                    domens += 1;
                    if (array[i][1] == 0)
                        indexses.add(i - 1);
        ...
        for (int i = 0; i < 2 * domens.intValue(); i++) {
            ...
            xcoordinates.add(-array[indexses.get(i)][1] / (array[indexses.get(i) + 1][1] - array[indexses.get(i)][1]) * (array[indexses.get(i) + 1][0] - array[indexses.get(i)][0]) + array[indexses.get(i)][0]);
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            if (array[i][0] >= xcoordinates.get(k) && array[i][0] < xcoordinates.get(k + 1)) {
// Преобразовать значения (x,y) в точку на экране point
                if (maxy < array[i][1]) {
                    maxy = array[i][1];
                }
                ...
                if (array[i - 1][0] <= xcoordinates.get(k)) {
// Первая итерация цикла - установить начало пути в точку point
                    integral[k / 2] += Math.abs((array[i][0] - xcoordinates.get(k)) * array[i][1] / 2);
                    canvas.setColor(Color.red);
                    ...
                    point = xyToPoint(array[i][0], array[i][1]);
                    path.lineTo(point.getX(), point.getY());
                }
                if (array[i + 1][0] >= xcoordinates.get(k + 1)) {
// Не первая итерация цикла - вести линию в точку point
                    Point2D.Double point = xyToPoint(array[i][0], array[i][1]);
                    path.lineTo(point.getX(), point.getY());
                    point = xyToPoint(xcoordinates.get(k + 1), 0);
                    path.lineTo(point.getX(), point.getY());
                    integral[k / 2] += Math.abs(array[i][1] / 2 * (xcoordinates.get(k + 1) - array[i][0]));
                    path.closePath();
                ...
                if(!(array[i + 1][0] >= xcoordinates.get(k + 1))&&!(array[i - 1][0] <= xcoordinates.get(k))) {
                    integral[k / 2] += Math.abs((array[i][0] - array[i - 1][0]) * (array[i][1] + array[i - 1][1]) / 2);
                    Point2D.Double point = xyToPoint(array[i][0], array[i][1]);
                    path.lineTo(point.getX(), point.getY());
                }
    ...
    protected void paintMarkers(Graphics2D canvas) {
        Double[][] arr;
        if (this.toCountSize != null) {
            arr = this.toCountSize;
        }else {
            arr = this.graphicsData;
        }
