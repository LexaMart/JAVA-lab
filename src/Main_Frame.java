...
public class Main_Frame extends JFrame {
    ...
    public Double formula1(Double x, Double y, Double z)
    {
        ...
        return ((Math.sin(Math.PI * y * y)) + Math.log10(y * y))/(Math.sin(Math.PI * z * z) + Math.sin(x)
                + Math.log10(z * z) + (x * x) + Math.exp(Math.cos(z * x)));
    }
    public Double formula2(Double x, Double y, Double z)
    {
        return (Math.pow(Math.cos(Math.exp(y)) + Math.exp(y*y) + Math.pow(1/x, 1/2), 1/4)) /
                (Math.pow(Math.cos(Math.PI * z * z * z) + Math.log10(Math.pow(1+z,2)), Math.sin(y))) ;
    }
